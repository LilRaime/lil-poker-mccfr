...
pub fn evaluate_7cards(hole: &[Card; 2], board: &[Card]) -> u64 {
    ...
    if let Some(f_suit) = flush_suit {
        let flush_cards: Vec<u64> = all_cards
            .iter()
            .filter(|c| c.suit as usize == f_suit)
            .take(5)
            .map(|c| c.rank as u64)
            .collect();
    ...
    if let Some(&t) = trips.first() {
        let kickers: Vec<u64> = (0..13)
            .rev()
            .filter(|&r| r as u64 != t && rank_counts[r] > 0)
            .take(2)
            .map(|r| r as u64)
            .collect();
        return (3 << 32)
    ...
    if let Some(&p) = pairs.first() {
        let kickers: Vec<u64> = (0..13)
            .rev()
            .filter(|&r| r as u64 != p && rank_counts[r] > 0)
            .take(3)
            .map(|r| r as u64)
            .collect();
        return (1 << 32)
